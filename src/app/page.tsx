...
export default async function DashboardOverview() {
  ...
  const canViewPurchasing = hasPermission(role, 'prCreate');
  const canViewCosting = hasPermission(role, 'viewJobCosting');
  const canViewBilling = hasPermission(role, 'invoiceRecordPayment') || canViewCosting;
  ...
  return (
    <div className="space-y-8 sm:space-y-10">
      ...
      <section aria-labelledby="workflow-heading">
        ...
        <EndToEndWorkflowVisualizer role={role} counts={data.counts} />
      </section>

      {(canViewJobOrders || canViewPurchasing) && (
          ...
          <div className="grid grid-cols-1 gap-3 sm:grid-cols-3 sm:gap-4">
            {canViewBilling && (
              <MoneyCard
                ...
                value={formatPeso(data.financials.billedCents)}
              />
            )}
            {canViewCosting && (
              <>

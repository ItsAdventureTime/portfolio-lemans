...
## 2. Four Isolated Target Environments (Verified Phase 3 Status)

| Environment Parameter   | 1. `local-demo`                            | 2. `local-prodlike`                         | 3. `remote-demo`                     | 4. `remote-production`               |
| ----------------------- | ------------------------------------------ | ------------------------------------------- | ------------------------------------ | ------------------------------------ |
| **Status**              | **Verified (200 OK)**                      | **Verified (200 OK)**                       | Prepared (Quadlets)                  | Prepared (Quadlets)                  |
| **Target Host**         | macOS (Apple Silicon arm64)                | macOS (Apple Silicon arm64)                 | Remote Linux Server                  | Remote Linux Server                  |
| **Podman Command**      | `podman machine start` + `podman compose`  | `podman machine start` + `podman compose`   | Systemd User Quadlet                 | Systemd User Quadlet                 |
| **Image Tag Standard**  | `latest-slim`                              | `lts-slim`                                  | `latest-slim`                        | `lts-slim`                           |
...
# Demo
podman compose -f docker-compose.yml -p lemans-demo up -d --build

# Prodlike
podman compose -f docker-compose.prodlike.yml -p lemans-prodlike up -d --build

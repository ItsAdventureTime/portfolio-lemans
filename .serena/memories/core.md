...
- Local execution and validation run through the initialized Docker Sandbox with Docker via `jk-sbx-project exec`; remote VPS services use rootless Podman Quadlets for runtime only.
- Remote deployment builds the verified target-platform images in the Sandbox, exports a checksum-verified Docker archive, then the VPS imports it with `podman load`; it does not build or compile on the VPS.
- One Job Order is the operational source of truth for quotation, procurement, OPEX, billing, collections, and profitability.
- Work stays on `main`; local Git uses `git`, remote GitHub operations use `gh` over authenticated HTTPS.

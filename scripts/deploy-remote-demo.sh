...
echo "[5/6] Creating remote environment, secret, and release manifest..."
...
ssh "${REMOTE_USER}@${REMOTE_HOST}" <<'REMOTE_SCRIPT'
  ...
  if [[ "\$(curl -s -o /dev/null -w '%{http_code}' "http://127.0.0.1:${APP_PORT}/lemans/demo" 2>/dev/null || true)" != "200" ]]; then
